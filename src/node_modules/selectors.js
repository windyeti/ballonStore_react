...
export const getGoods = (state, ownProps) => {
    // console.log('state.goodsPage.ids', state.goodsPage.ids);
    const categoryActive = getActiveCategoriesId(ownProps);
    console.log('ownProps', ownProps);
    return R.compose(
        R.when(R.always(categoryActive), R.filter(good => {
            return good.category === categoryActive
            }
        )),
        R.map(id => getGoodsById(id, state))
    )(state.goodsPage.ids);}
