...
class Catalog extends React.Component {
    ...
    renderGoods() {
        ...
        const arrArrsHeFoilByCategory = getArrArrsGoodsByCategory(arrArrsSortGoodsBySetId, '3');
        const arrArrsAirFoilByCategory = getArrArrsGoodsByCategory(arrArrsSortGoodsBySetId, '4');
        const arrArrsDecorByCategory = getArrArrsGoodsByCategory(arrArrsSortGoodsBySetId, '5');
        const allGoods = R.concat(
            this.renderBallon(arrArrsHeLatexByCategory),
            this.renderBallon(arrArrsAirLatexByCategory),
            this.renderBallon(arrArrsHeFoilByCategory),
            this.renderBallon(arrArrsAirFoilByCategory),
            this.renderBallon(arrArrsDecorByCategory)
        );

        let arrResult = [];

        const arrArrsCouple = arr => {
            let newArr = [];
            let result = [];
        ...
        // { allGoods.length && arrArrsCouple(allGoods) }
        if(allGoods.length) { arrArrsCouple(allGoods) }

        return arrResult
    }
    renderGoodsInRow(arrCouple) {
        return (
            arrCouple.map(coupleGoods => {
                return (
                    <div className="row">
                        { coupleGoods[0] }
                        { coupleGoods[1] }
            ...
            <div className='col-xs-12'>
                <div className="row">
                    ...
                    <div className="col-sm-8">
                        { this.renderGoodsInRow( this.renderGoods() ) }
                    </div>
                </div>
            </div>
        )
